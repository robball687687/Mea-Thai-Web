...
const spiceColors = ["#9ca3af", "#22c55e", "#f59e0b", "#ef4444"];
...
export default function MenuItemCard({ item, onObserve }) {
  ...
  const image = item?.displayImage || item?.imageUrl || FALLBACK_IMAGE;
          ...
          <div className="flex items-center gap-1">
            {[...Array(spice)].map((_, i) => (
              <span
                ...
                style={{
                  background: spiceColors[Math.min(i + 1, spiceColors.length - 1)],
                }}
              />
            ))}
